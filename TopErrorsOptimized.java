...
public class TopErrorsOptimized {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            queue.add(new LogEntry(line));

            while((line = reader.readLine()) != null) {
                //only process last 10 minutes
                //if last element in the queue is within last 10 minutes of NOW (curr element)
                    //put current in queue
                
                //if not, then remove the head of the queue

                LogEntry current = new LogEntry(line);
                Long now = current.unixTimeStamp;
                Long cutOff = now - queue.getFirst().unixTimeStamp;
                if(cutOff <= 600) {
                    queue.add(current);
                } 
                else {
                    ...
                    //remove more
                    cutOff = now - queue.getFirst().unixTimeStamp;
                    while(cutOff > 600) {
                        queue.removeFirst();
                        cutOff = now - queue.getFirst().unixTimeStamp;
                    }
        ...
        HashMap<String, Integer> counts = getCounts(queue);
